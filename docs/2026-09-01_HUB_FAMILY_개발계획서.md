...
| 디자인 | **MEDIC Soft Wellness** (2026-09-08) | 아이콘 30종 · 배경 5종 · 로고. 팔레트는 에셋 팩 README 를 따른다 |
| 앱 셸 | **Capacitor 6** | `android/`, `ios/` 네이티브 프로젝트 분리 관리 |
| 알림 | 푸시(FCM / APNs) **주** + 로컬 알림(Capacitor Local Notifications) **보험** | 서버가 항상 푸시하고, 통신 끊김 대비로 로컬도 예약 — 8.5 |
| 웹서버 | Apache 2.4 (리버스 프록시, 443 종단) | 기존 서버 자산 활용 |
| 컨테이너 | **Docker Compose** | api / worker / db / redis |
| OS | Ubuntu Server | |
| CI | GitHub Actions | lint · test · 이미지 빌드 |

> 프론트엔드는 React + TypeScript + Vite 로 확정했다 (2026-09-01).
...
hubfamily/
├─ docs/                       # 기획 문서, 계획서, ERD
│  ├─ 2026-09-01_HUB_FAMILY_개발계획서.md
│  ├─ HUB_FAMIL.pptx
│  └─ 앱구성_MEDIC.pdf
├─ backend/
│  ├─ app/
...
| `notification_log` | user_id, kind, delivery(local/push), channel, scheduled_at, sent_at, status, error, route | 알림 발송 이력. "안 울렸어요" 대응 (8.5.8) |
...
- `photo_url`은 DB에 경로만, 실제 파일은 호스트 볼륨(`/home/hubfamily/uploads`) 저장. MVP는 로컬 볼륨, 확장 시 S3 호환 스토리지.
...
| GET | `/notifications/plan` | 단말이 앞으로 2주치 로컬 알림을 예약하려고 받아간다. 서버가 `local_id`(int32)를 발급한다 (8.5.5) | M3 |
| POST | `/notifications/report` | 단말이 로컬 알림 예약·발화 결과를 올린다. 중복 방지용이 아니라 이력 확인용 (8.5.11) | M3 |
...
| S4 | **기분 체크** | `/checks/moods` | 아침·점심·저녁 행 × **이모지 3개**(좋아요·보통이에요·힘들어요) 즉시 선택 |
...
### 7.3.1 기분 체크는 3단계로 유지 (2026-09-08 결정)

Soft Wellness 시안은 기분을 5단계(아주 좋아요·좋아요·괜찮아요·조금 힘들어요·힘들어요)로
그렸으나 **3단계를 유지한다.**

- 어르신에게 다섯 중 고르라는 것은 부담이다. 화면당 행동을 줄이는 방향(9장)과 어긋난다
- 하루 세 번 × 다섯 선택지면 버튼이 15개다. 터치 타깃 56px 를 지키면 화면을 넘긴다
- 3단계로도 "좋아짐 / 나빠짐" 추이는 충분히 읽힌다

`MoodValue` enum(`good` · `normal` · `bad`)은 그대로 둔다. 나중에 늘리려면
값을 추가하고 마이그레이션하면 되지만, 이미 쌓인 3단계 기록과 섞이는 문제를 먼저 풀어야 한다.

### 7.4 설정 화면 없음 — 결정 완료 (2026-09-08)

최종안에 설정 화면이 없다. 계획서 9장이 수용 기준으로 둔 글자 크기 배율과 음성 안내를
어디서 바꿀지 정해야 했고, **기기 설정을 따르는 것**으로 결론냈다.

- 글자 크기 → 기기 설정 (9.1). CSS 를 `rem` 으로 전환 완료
- 음성 안내 → 기기 스크린 리더 (9.2). 자체 TTS 를 넣지 않는다

자녀가 부모님 설정을 대신 만지는 안(자녀 권한 `/settings`)과 부모 메인에 설정 진입을
추가하는 안은 모두 버렸다. 전자는 남이 내 글자 크기를 정하는 셈이고,
후자는 어르신이 실제로 들어가 바꾸지 않는다.
...
| 단계 | 시점 | 동작 |
|---|---|---|
| L0 | 예정 시각 | 어르신 알림 "약 드실 시간이에요" |
| L1 | +30분 미응답 | 어르신 재알림 (소리·진동 강화) |
| L2 | +2시간 미응답 | 어르신 3차 알림 + 보호자 앱에 `missed_med` 표시(자녀에게는 푸시하지 않음) |
| L3 | 당일 미응답 마감 | `medication_logs.status = missed` 확정, 일일 리포트 반영 |

L0~L2 는 **서버 푸시로 보내고 로컬 알림도 함께 예약한다** — 8.5.1.
어르신이 "복용함"을 누르면 서버는 이후 단계 발송을 멈추고, 단말은 L1·L2 로컬 예약을 취소한다.
...
### 8.5 알림 전달 방식 (2026-09-08 확정 → 2026-09-09 수정)

> **2026-09-10 추가: iOS 는 FCM 이 아니라 애플(APNs)로 직접 보낸다.**
> Capacitor 푸시 플러그인이 iOS 에서 주는 것은 APNs 토큰이라 FCM 이 못 받는다. 앱을 갈아엎는 대신
> 서버가 플랫폼을 보고 갈라 보낸다 (`services/apns.py`). 안드로이드는 채널이 알람음을 정하고,
> iOS 는 알림마다 `medic_alarm.wav` 와 `interruption-level: time-sensitive` 를 얹는다.
> 무음 스위치까지 뚫는 Critical Alerts 는 애플 승인이 필요해 쓰지 않는다. 자세한 절차는 `mobile/README.md`.

> **2026-09-09 결정: 로컬 알림을 뺐다. 알림은 서버 푸시 하나다.**
> 실기기에서 같은 약이 로컬(정각)·푸시(최대 5분 뒤) 두 번 울리는 것이 감수하기 어려웠다.
> 대신 워커 주기를 1분으로 줄여 정각에 붙이고, 복약 푸시는 알람 채널(알람 볼륨 · 30초 벨)로 보낸다.
> 통신이 끊긴 폰에는 알림이 안 온다 — 이 트레이드오프를 받아들였다. 아래 8.5.1~8.5.5 의 로컬 예약 설명은
> 서버 API(`/notifications/plan`) 로만 남아 있고 앱은 쓰지 않는다. 재알림(L1·L2) 은 `MED_ESCALATION` 로 보류.

**원칙(당초): 서버 푸시가 주(主), 로컬 알림은 통신이 끊겼을 때의 보험.**

로컬 알림이 더 정확하다는 점은 맞다 — 한 번 예약되면 통신·서버와 무관하게 OS 가 울려 준다.
그런데 **예약은 부모 앱이 열려야 걸린다.** 어르신이 앱을 안 여는 것이 이 서비스의
가장 큰 리스크(14장)이고, 앱을 안 열면 예약이 없어 알림도 없고, 알림이 없으니 더 안 열게 된다.

그래서 **서버가 모든 알림을 푸시로 보낸다.** 예약 상태를 신경 쓰지 않는다.
로컬은 통신이 끊겼을 때를 위해 함께 깔아 둔다.

#### 8.5.1 전달 경로

```
서버가 알림 시각마다 푸시를 쏜다            ← 항상. 예약 여부와 무관
부모 앱이 열릴 때 로컬 알림도 예약해 둔다     ← 통신 끊김 대비
```

| 상황 | 결과 |
|---|---|
| 통신 정상 | 푸시가 온다 (로컬도 걸려 있으면 두 번) |
| 통신 끊김 | 로컬이 울린다 |
| 앱을 한 번도 안 열었다 | 푸시만 온다 |
| 서버 장애 | 로컬이 울린다 (예약이 있다면) |

**어느 쪽이든 최소 하나는 울린다.** 서버가 단말의 예약 상태를 알 필요가 없어 로직도 단순하다.

#### 8.5.2 중복은 감수한다

푸시와 로컬이 같이 울릴 수 있다. **중복 방지 장치를 만들지 않는다.**

서버가 "이 단말에 예약이 걸려 있는가"를 알려면 단말이 보고해야 하는데, 앱이 종료돼 있으면
보고를 못 한다. 그 예외를 다루는 비용이 중복으로 인한 불편보다 크다.

알림이 두 번 오는 것보다 **한 번도 안 오는 것이 훨씬 나쁘다.**

#### 8.5.3 알림별 정리

| 알림 | 대상 | 푸시 | 로컬 |
|---|---|---|---|
| 복약 L0 · L1 · L2 | 어르신 | ✅ 서버가 발송 | ✅ 예약 |
| 일정 알림 (사전·정각) | 어르신 | ✅ 서버가 발송 | ✅ 예약 |
| 이상 징후 | 자녀 | ✅ | — (서버만 아는 사건) |
| 하루 요약 | 자녀 | ✅ | — |
| 일정·복약 시간 변경 | 어르신 | 무음 푸시 | 단말이 재예약 |

#### 8.5.4 로컬 예약 — 첫 로그인 때 반드시 건다

**첫 로그인 시점이 가장 확실하다.** 자녀가 옆에서 도와주는 시점이라 앱이 확실히 열린다.
그때 앞으로 2주치를 깔아 두면, 이후에는 알림을 누를 때마다 앱이 열리면서 자동으로 갱신된다.

- **iOS 는 앱당 로컬 알림 64개가 한계다.** 초과분은 에러 없이 조용히 예약되지 않는다.
  복약 3회 × 7일 = 21개, 일정까지 합치면 금방 찬다 → **예약 창을 2주로 제한**하고 서버가 잘라서 준다
- 갱신 시점: 앱 실행 시 · 알림을 눌러 진입했을 때 · 안드로이드는 `WorkManager` 주기 작업
- 안드로이드는 `setExactAndAllowWhileIdle` 로 Doze 상태에서도 정확히 울린다

#### 8.5.5 알림 ID — 서버가 발급한다

Capacitor 로컬 알림 ID 는 **32비트 int** 여야 한다. 우리 `schedules.id` 는 UUID 이므로
`scheduleId × 100 + 순번` 같은 계산을 쓸 수 없다.

**서버가 int 범위의 `local_id` 를 발급하고 단말은 받은 대로 예약한다.**

```json
GET /notifications/plan?days=14
{
  "items": [
    { "local_id": 1042, "at": "2026-09-19T14:00:00+09:00",
      "title": "내일 병원 일정이 있어요", "body": "춘천성심병원 진료",
      "channel": "schedule", "route": "/s/schedule/582" },
    { "local_id": 1043, "at": "2026-09-20T13:00:00+09:00",
      "title": "1시간 후 병원 일정이 있어요", "body": "춘천성심병원 진료",
      "channel": "schedule", "route": "/s/schedule/582" }
  ],
  "revoked_ids": [1011, 1012]
}
```

복약과 일정을 한 응답에 섞어 내려보내 단말 로직을 하나로 유지한다.
일정이 수정·삭제되면 그 `local_id` 들이 `revoked_ids` 로 내려온다.

#### 8.5.6 사전 알림 시각

자녀가 일정을 등록할 때 고른다. 복수 선택 가능.

`정각` · `10분 전` · `30분 전` · `1시간 전` · `2시간 전` · `하루 전`

하나의 일정에 여러 알림이 붙으므로 `local_id` 는 알림 하나당 하나씩 발급한다.

시각은 **KST 고정**으로 계산한다. 해외 거주 자녀·기기 타임존 변경은 MVP 범위 밖이다.

#### 8.5.7 안드로이드 알림 채널

채널은 한 번 만들면 사용자가 끌 수 있고 **앱이 되돌릴 수 없다.** 처음부터 나눠야
"하루 요약만 끄고 약 알림은 켜두기"가 가능하다.

| 채널 ID | 이름 | 중요도 | 소리·진동 |
|---|---|---|---|
| `medication` | 약 복용 알림 | HIGH | 소리 + 진동 |
| `anomaly` | 이상 징후 | HIGH | 소리 + 진동 |
| `schedule` | 일정 | DEFAULT | 소리 |
| `report` | 하루 요약 | LOW | 무음 |
...
| 권한 | 대상 | 요청 시점 |
|---|---|---|
| `POST_NOTIFICATIONS` | Android 13+ | **첫 로그인 직후**, 왜 필요한지 한 줄 보여주고 요청 |
| `USE_EXACT_ALARM` | Android 12+ | 복약 알림을 처음 예약할 때 |
| 알림 허용 | iOS | 첫 로그인 직후 |
| 배터리 최적화 예외 | Android | 생활 신호 수집을 켤 때 안내 (8.3) |
| 카메라 | Android · iOS | 식사 사진을 **찍을 때** (S2 `사진 추가`) |
| 사진 보관함 | Android 13+ `READ_MEDIA_IMAGES` · iOS 사진 접근 | 앨범에서 **고를 때** |

**카메라·사진 권한은 필요한 순간에만 묻는다.** 식사 체크는 사진 없이도 끝나는 흐름이므로
(사진은 선택 항목), 처음부터 요청하면 거부당하고 그 뒤로 못 쓰게 된다.
`사진 추가`를 누른 그 시점에 묻고, 거부해도 체크 자체는 정상 완료된다.

iOS 는 `Info.plist` 에 사용 목적 문구가 있어야 심사를 통과한다.
`NSCameraUsageDescription` · `NSPhotoLibraryUsageDescription` 에
"식사 기록에 사진을 남기기 위해 사용합니다" 를 넣는다.

- 앱 첫 화면에서 바로 묻지 않는다. **왜 필요한지 아는 상태에서 물어야 승낙률이 높다.**
- 거부한 경우 어르신 홈에 "알림이 꺼져 있어요" 배너를 띄우고, **자녀 화면에도 표시**한다.
  자녀가 대신 챙길 수 있어야 한다.
- `USE_EXACT_ALARM` 은 알람·리마인더 앱에만 허용된다. **Play 심사에서 소명이 필요**하므로
  복약 관리라는 용도를 미리 정리해 둔다.

#### 8.5.9 알림을 눌렀을 때

payload 의 `route` 로 이동한다. 앱이 꺼져 있다 실행되는 경우(cold start)도 같은 경로를 태운다.
...
- **22:00~07:00 KST 에는 하루 요약·일반 알림을 보내지 않는다.** 다음 날 아침으로 미룬다
- **이상 징후는 예외다.** 그 시간에 알리는 것이 목적이다
- 복약·일정은 자녀가 설정한 시각이므로 그대로 따른다

#### 8.5.11 발송 이력

"안 울렸어요"라는 문의에 답할 수 있어야 한다. `notification_log` 에 서버 발송분을 남긴다.
로컬 알림은 단말이 띄우므로 서버가 모른다 — 단말이 예약·발화 결과를 배치로 올린다
(생활 신호와 같은 주기). 보관 90일.

#### 8.5.12 검증이 필요한 항목

M3 착수 시 **실기기로** 확인한다. 문서로 판단하지 않는다.

- 기기 재부팅 후 로컬 알림 예약이 복원되는가 (안 되면 네이티브 부팅 리시버 추가)
- 앱 업데이트 후 예약이 유지되는가
- 제조사 절전(샤오미·화웨이·삼성)에서 푸시와 로컬이 각각 도달하는가
- iOS 집중 모드에서의 동작
...
| 항목 | 필요 시점 | 비고 |
|---|---|---|
| Firebase 프로젝트 + `google-services.json` | **M3 착수 전** | 패키지명 `kr.co.mangotree.hubfamily` |
| Apple 개발자 계정 + APNs 인증키(`.p8`) | iOS 착수 시 | 연 $99 |
| `@capacitor/local-notifications` · `@capacitor/push-notifications` | M3 | |

인증서·키 파일은 저장소에 넣지 않는다 (4.2 `.gitignore`).
...
사업계획서 차별화 요소 및 WO 전략을 개발 기준으로 고정한다. **취향이 아니라 수용 기준이다.**

| 항목 | 기준 |
|---|---|
| 본문 | 최소 **18px**(`1.125rem`), 주요 숫자·버튼 **24px**(`1.5rem`) 이상 |
| 글자 크기 조절 | **기기 설정을 따른다.** 앱 안에 배율 설정을 두지 않는다 (2026-09-08 결정 · 9.1) |
| 터치 타깃 | 최소 **56px**(`3.5rem`), 버튼 간 간격 12px 이상 |
| 화면당 행동 | 주요 행동 **1~2개**. 어르신 화면은 스크롤 없이 보이는 범위에 핵심 버튼 배치 |
| 색 대비 | **WCAG AA(4.5:1) 이상.** 파스텔은 배경에만, 글자는 진한 색을 얹는다 |
| 상태 표시 | 색만으로 구분하지 않는다. `완료`·`기록 없음` 같은 문구를 항상 함께 쓴다 |
| 확인 절차 | 체크는 1탭 완료. 실수 시 되돌리기 제공 |
| 음성 안내 | **자체 TTS 를 넣지 않는다.** 기기 스크린 리더가 읽도록 마크업한다 (9.2) |
| 전화 걸기 | `tel:` 직결, 중간 단계 없음 |
| 용어 | 전문 용어·영문 금지. "로그인 세션 만료" → "다시 시작해 주세요" |

### 9.1 글자 크기 — 기기 설정을 따른다

최종 화면 구성(7장)에 설정 화면이 없다. 앱 안에서 배율을 바꿀 자리를 새로 만드는 대신
**기기의 글자 크기 설정을 그대로 따른다.** 어르신은 이미 기기에서 글자를 키워 쓰는 경우가 많고,
앱마다 따로 설정하게 만드는 편이 오히려 부담이다.

그러려면 **글자와 터치 영역이 `rem` 이어야 한다.** `px` 로 박으면 기기 설정이 반영되지 않는다.

- `webapp/src/shared/tokens.css` 의 `--text-*` 와 `--touch-min` 은 전부 `rem`
- 여백(`--gap-*`)은 글자가 아니므로 `px` 로 둔다
- 안드로이드 웹뷰는 시스템 글꼴 크기를 자동 반영한다
- **iOS 웹뷰는 자동 반영되지 않는다.** M5 에서 네이티브 `fontScale` 을 읽어
  루트 `font-size` 에 반영한다 (`webapp/src/native/bridge.ts`)
- `user_settings.font_scale` 컬럼은 남겨 두되 쓰지 않는다. 되살릴 일이 생기면 그때 붙인다

### 9.2 음성 안내 — 스크린 리더에 맡긴다

설정이 없는데 자체 TTS 가 항상 재생되면 끌 방법이 없다. 기기의 스크린 리더
(안드로이드 TalkBack · iOS VoiceOver)가 읽도록 접근성 마크업을 제대로 하는 쪽으로 간다.

- 의미는 항상 **글자**가 전달한다. 아이콘은 장식이므로 `aria-hidden`
- 모든 버튼에 읽을 수 있는 이름을 준다 (아이콘만 있는 버튼은 `aria-label`)
- 상태 변화는 `role="status"`, 오류는 `role="alert"` 로 알린다
- 현재 위치는 `aria-current="page"`(탭), 선택 상태는 `aria-pressed`
- 화면 제목은 `h1` 하나. 시각적 크기가 아니라 구조로 순서를 잡는다

### 9.3 반응형

주 대상은 폰이지만 브라우저에서도 열린다.

| 폭 | 처리 |
|---|---|
| ~360px | 여백·타일 축소. 상태 알약이 제목을 밀지 않도록 줄바꿈 허용 |
| 361~599px | 기본 |
| 600px~ | 앱 컬럼(500px)을 화면 가운데 세우고 바탕을 한 톤 낮춤. 배치는 폰과 동일 |
| 가로 모드 · 높이 520px 이하 | sticky 해제 — 상·하단이 화면을 다 먹지 않게 |

본문은 가로로 밀리지 않는다(`overflow-x: hidden`). 부모님 전환 칩만 가로 스크롤을 허용한다.
...
## 12. MVP 이후 확장 대비 (2026-09-02 결정)

MVP 범위는 아니지만, **나중에 붙이면 비싼 것**만 지금 구조에 반영한다.

### 12.1 지금 넣는 것 / 나중에 넣는 것

| 항목 | 시점 | 이유 |
|---|---|---|
| `users.email` | **M1** | 나중에 추가하면 기존 사용자 전원에게 다시 받아야 한다 |
| 동의 항목 세분화 | **M1** | 이메일 수신·AI 처리 동의는 법적으로 소급이 안 된다 |
| AI 호출 지점 단일화 | **M1 (구조만)** | 호출을 서비스 한 곳으로 모으면 계측을 나중에 한 번에 붙인다 |
| `ai_usage` 테이블 | AI 도입 시 | 마이그레이션으로 추가하면 된다 |
| `outbox_emails` | M4 | 위와 같음 |
| 결제 연동 | 별도 마일스톤 | 아래 12.3 |

### 12.2 리포트 메일 — 네이버웍스 SMTP

`smtp.worksmobile.com:465` (SSL). 계정은 `.env` 의 `SMTP_*` 에만 둔다.

- **네이버웍스는 인증 계정 주소로만 발신된다.** From 을 사용자 주소로 바꿔 보내면 거부된다.
  표시 이름만 `MAIL_FROM_NAME` 으로 바꾸고, 답장은 `Reply-To` 로 받는다.
- **이 서버에서 직접 발송하지 않는다** — 사이트 100여 개가 공유하는 IP다.
  우리가 스팸으로 분류되면 다른 사이트 메일까지 영향을 받는다. 외부 SMTP(네이버웍스)를 경유한다.
- 발송량이 늘면 네이버웍스 일일 한도를 확인해야 한다. 사용자 증가 시 전용 발송 서비스로 전환.

구조는 **outbox 패턴**이다. 리포트 생성과 발송을 분리해, 메일 서비스가 죽어도 리포트는 남고 나중에 재발송된다.

```
리포트 생성(21:00) → outbox_emails INSERT (pending)
                          ↓
              worker 가 주기적으로 집어서 발송
                          ↓
              성공 → sent  /  실패 → 재시도 (최대 3회)
```

서버에서 실제 발송이 되는지 M4 전에 확인한다 — 공용 호스트라 465 아웃바운드가 막혀 있을 수 있다.

```bash
docker compose exec -T api python scripts/send_test_mail.py 받는사람@example.com
```

### 12.3 결제 — 웹 결제 (추후 변경 가능)

MVP 는 결제를 붙이지 않는다. 15일 체험 상태만 `subscriptions` 로 관리한다.

방향은 **웹 PG** 다. 다만 앱에서 구독을 팔면 스토어 인앱결제가 강제될 수 있어, 출시(M6) 전까지 결론이 필요하다.

| 경로 | 수수료 | 월 9,900원 실수령 |
|---|---|---|
| 웹 결제(PG) | 2~3% | 약 9,600원 |
| 인앱결제 | 15~30% | 6,930 ~ 8,415원 |

### 12.4 AI 사용량 — 도입 시 반드시 계측

**AI 기능은 현재 보류.** 도입하게 되면 호출당 비용이 발생하므로, 사용량과 금액을 사용자 단위로 추적해야 구독가를 검증할 수 있다.

호출 한 건마다 남길 것: `family_id`, `user_id`, `feature`, `model`, `input_tokens`, `output_tokens`, `cache_read_tokens`, `cost_krw`(호출 시점 단가로 계산해 고정), `latency_ms`, `status`.

참고 추정치 — 일일 리포트 1건을 입력 1,500 / 출력 300 토큰으로 잡을 때:

| 방식 | 월 30건 | 구독가 대비 |
|---|---|---|
| Opus 5 | 약 600원 | 6% |
| Opus 5 + Batch API | 약 300원 | 3% |
| Haiku 4.5 | 약 120원 | 1.2% |

- **일일 리포트는 Batch API 대상이다.** 밤에 한 번에 돌리는 작업이라 실시간성이 필요 없고 비용이 반값이다.
- 시스템 프롬프트가 전 사용자 공통이므로 프롬프트 캐싱으로 입력 비용을 더 줄인다.
- 이상 징후 분석만 실시간이라 배치를 못 쓴다. 호출 빈도가 낮아 부담은 적다.
...
| **M3. 알림·일정** | 10/20~11/7 (3주) | **로컬 알림 예약**(7일치·재예약), 알림 채널·권한, FCM 연동, 복약 에스컬레이션, 일정 CRUD(G3·S5), 약 복용 시간 설정(G4), worker 스케줄러 |
...
| **알림 권한 거부 · 채널 끄기** | 복약 알림이 안 울려 서비스가 무력화 | 권한을 첫 화면이 아니라 필요한 시점에 이유와 함께 요청. 꺼져 있으면 어르신 홈과 **자녀 화면 양쪽에 표시**해 자녀가 대신 챙기게 한다 (8.5.4) |
...
- ~~서버 도메인~~ → `hubfamily.mangotree.co.kr` (배포 완료, HTTP)
- ~~휴대폰 인증 SMS 공급사~~ → MVP 범위에서 제외 (1.4 참고). 실사용자 전 재검토
- ~~리포트 메일 발송 수단~~ → 네이버웍스 SMTP (12.2)
- ~~결제 방향~~ → 웹 PG, 추후 변경 가능 (12.3)
